# Upload customers via Zed using single CSV file

## Combined customers CSV file.

To import customers via Zed UI, go to Zed panel `Customers->Customers` and click `Import Customers` button.

Files:

- example valid csv file: [combined_customers](files/combined_customers.csv)
- example invalid headers file: [combined_customers_invalid_neaders](files/combined_customers_invalid_headers.csv)
- example invalid data file: [combined_customers_invalid_data](files/combined_customers_invalid_data.csv)
...
## Import customers via Zed UI.

A diagram showing the general operation of the customer import functionality <br>

![CSV import from file - flow diagram](./img/customer_csv_import_diagram.png)
...
![Headers validation flow](./img/customer_csv_headers_validation.png)
...
![Data validation flow](./img/customer_csv_data_validation.png)
...
#### 25. `admin_gender` column.

* Description:
    - Admin's gender. It reflects gender in `spy_customer`.
* Field properties & validation rules:
    - the field is required
    - the only options that can be entered in the fields are `Male` and `Female`

## Technical info

1. Translations for text fields in Zed can be found in `src/Pyz/Zed/Translator/data/Customer`.
...
4. Customers do not have any id (as is the case with e.g. unitAdress) and are saved based on e-mail - there is e-mail validation in the Customer.php class - if one already exists in the database, the customer will not be created.
5. To run the tests, use these two commands:
```bash
codecept build -c tests/PyzTest/Zed/CustomerCsvImport
```
```bash
codecept run -c tests/PyzTest/Zed/CustomerCsvImport
```
